...
/*** TO BE DONE 2.2 START ***/
	if (!gmtime_r((const time_t *)&now_t, &now_tm))
		fail_errno("gmtime_r");
/*** TO BE DONE 2.2 END ***/
...
/*** TO BE DONE 2.2 START ***/
		if (!gmtime_r((const time_t *)&file_modification_time, &file_modification_tm))
					fail_errno("gmtime_r");
...
/*** TO BE DONE 2.2 START ***/
				...
				if (strptime(option_val, "%a, %d %b %Y %H:%M:%S GMT", &since_tm) == NULL)
						continue;
...
/*** TO BE DONE 2.2 END ***/
		...
		if (access(filename, R_OK) != 0 || stat(filename, stat_p) < 0) {
			...
			if (MethodIsConditional(http_method)) {
				 ...
				 ***/
/*** TO BE DONE 2.2 START ***/
				http_method = stat_p->st_mtime <= timegm(&since_tm) ? METHOD_NOT_CHANGED : (http_method & METHOD_NOT_CHANGED);
/*** TO BE DONE 2.2 END ***/
